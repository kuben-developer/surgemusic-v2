...
interface CreateFolderDialogProps {
  onCreateFolder: (folderName: string) => Promise<unknown>;
}
...
export function CreateFolderDialog({ onCreateFolder }: CreateFolderDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!/^[a-zA-Z0-9_-]+$/.test(folderName)) {
      toast.error("Folder name can only contain letters, numbers, hyphens, and underscores");
      return;
    }
      ...
      <DialogContent className="sm:max-w-[425px]">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            ...
            <DialogDescription>
              Enter a name for your new clipper folder. Use letters, numbers, hyphens, or underscores.
            </DialogDescription>
          </DialogHeader>
