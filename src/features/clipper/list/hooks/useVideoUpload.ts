...
export function useVideoUpload() {
  const generateUploadUrl = useAction(api.app.clipperS3.generateUploadUrlSimple);
  const uploadVideoMutation = useMutation(api.app.clipperDb.uploadVideo);
  ...
  const uploadVideo = useCallback(
    async (file: File, folderId: FolderId, inputVideoName: string) => {
      try {
        ...
        // Create DB record and trigger background processing
        ...
        toast.success(`${inputVideoName} uploaded successfully`);
