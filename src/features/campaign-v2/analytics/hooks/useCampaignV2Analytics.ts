...
import type { CampaignV2AnalyticsData, DateRange } from "../types/analytics.types";

export function useCampaignV2Analytics(campaignId: string) {
  const [dateRange, setDateRange] = useState<DateRange>(30);
  const [isRefreshing, setIsRefreshing] = useState(false);

  const getCampaignAnalytics = useAction(api.app.bundleSocial.getCampaignAnalyticsWithMetadata);
  const [analyticsData, setAnalyticsData] = useState<CampaignV2AnalyticsData | null>(null);
  ...
  const fetchAnalytics = async (days: DateRange = dateRange) => {
    try {
      ...
      setAnalyticsData(data as CampaignV2AnalyticsData);
    } catch (err) {
