...
function parseCSV(content: string): OutputRecord[] {
  ...
  const postCaptionIndex = headers.indexOf('Post Caption');
  const campaignIdIndex = headers.indexOf('campaign_id');

  if (accountIndex === -1 || postCaptionIndex === -1 || campaignIdIndex === -1) {
  ...
  while (currentIndex < cleanContent.length) {
    ...
    const postCaption = fields[postCaptionIndex]?.trim();
    const campaignUrl = fields[campaignIdIndex]?.trim();

    if (!account || !campaignUrl) continue;
...
function main() {
  ...
  const outputPath = "scripts/captions-output.json"
